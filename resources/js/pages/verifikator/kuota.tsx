    ...
    function save() {
        ...
        router.patch(
            ...
            { quota_total: parsed },
            {
                ...
                onError: (errs) => setError(errs.quota_total ?? 'Gagal memperbarui kuota.'),
...
export default function VerifikatorKuota({ user = MOCK_USER, opds = MOCK_OPDS }: VerifikatorKuotaProps) {
    return (
        <MagangLayout user={user} title="Kelola Kuota OPD" active="kuota" navItems={verifikatorNav}>
            <Head title="Kelola Kuota OPD — E-Magang" />

            <div className="space-y-6">
                <div>
                    <h2 className="text-xl font-black text-[#12213e]">Kelola Kuota OPD</h2>
                    <p className="mt-1 text-sm text-slate-500">
                        Sebagai Admin Verifikator, Anda dapat mengubah kuota magang seluruh OPD. Kuota tidak boleh lebih kecil dari jumlah yang sudah terpakai.
                    </p>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5">
                    <div className="mb-1 flex items-center gap-1.5">
                        ...
                        <h3 className="text-sm font-bold text-[#12213e]">Daftar Kuota</h3>
                    </div>
                    <p className="mb-4 text-xs text-slate-500">Klik “Ubah” untuk menyesuaikan total kuota tiap OPD.</p>
