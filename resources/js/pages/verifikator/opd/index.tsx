 ...
 *
 *  Revisi baru: saat OPD dibuat, backend juga membuat akun admin OPD dan
 *  mengirim flash `generatedCredentials` → dialog salin kredensial.
 *  Tombol "Reset Password" me-regenerate password akun OPD (flash sama).
 * ========================================================================= */
...
/** Reset password akun admin OPD — konfirmasi inline, hasil via flash dialog. */
function ResetPasswordButton({ opdId }: { opdId: number }) {
    const [confirming, setConfirming] = useState(false);
    const [processing, setProcessing] = useState(false);
    ...
    if (!confirming) {
        return (
            <button
                type="button"
                onClick={() => setConfirming(true)}
                title="Buat password baru untuk akun admin OPD ini"
        ...
        <span className="inline-flex items-center gap-1.5">
            <button
                ...
                onClick={() => {
                    setProcessing(true);
                    router.post(
                        `/verifikator/opd/${opdId}/reset-password`,
                        {},
                        {
                            preserveScroll: true,
...
export default function OpdIndex({
    ...
    // Pencarian client-side: nama / kode / inisial OPD.
    const filtered = useMemo(() => {
        ...
        return list.filter(
            (opd) =>
                ...
                opd.code.toLowerCase().includes(q) ||
                (opd.inisial_opd ?? '').toLowerCase().includes(q) ||
                String(opd.kode_opd ?? '').includes(q),
        );
    }, [list, query]);
    ...
    return (
        ...
        >
            ...
            {/* Dialog kredensial akun OPD (flash generatedCredentials) */}
            ...
            <div className="space-y-6">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                            ...
                            kuota magangnya. Menambah OPD juga membuat akun
                            admin OPD secara otomatis.
                        </p>
                    </div>
                    <Link
                ...
                <div className="relative sm:w-72">
                    ...
                    <input
                        ...
                        placeholder="Cari nama / kode / inisial OPD…"
                                ...
                                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                    <div className="min-w-0 space-y-1">
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            {opd.kode_opd != null && (
                                                <span
                                                    title="Kode OPD (internal)"
                                                    className="rounded-md bg-slate-50 px-2 py-0.5 font-mono text-xs font-semibold text-slate-400 ring-1 ring-slate-200"
                                                >
                                                    #{opd.kode_opd}
                                                </span>
                                            )}
                                            {opd.inisial_opd && (
                                                <span
                                                    title="Inisial OPD (publik)"
                                                    className="rounded-md bg-[#e8f2fe] px-2 py-0.5 text-xs font-semibold text-[#106feb]"
                                                >
                                                    {opd.inisial_opd}
                                                </span>
                                            )}
                                    ...
                                    <div className="flex shrink-0 flex-wrap items-center gap-1">
                                        ...
                                        <ResetPasswordButton opdId={opd.id} />
                                        <DeleteButton
                                            opdId={opd.id}
                                            disabled={used > 0}
                                        />
