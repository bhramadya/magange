...
export default function FaqCreate({ user = MOCK_USER }: FaqCreateProps) {
    ...
    return (
        <MagangLayout user={user} title="Tambah FAQ" active="faq" navItems={verifikatorNav}>
            <Head title="Tambah FAQ — Verifikator" />

            <div className="mx-auto max-w-2xl space-y-6">
                <Link href="/verifikator/faq" className="inline-flex items-center gap-1.5 text-sm font-medium text-slate-500 transition hover:text-[#106feb]">
                    <ArrowLeft className="size-4" /> Kembali ke daftar
                </Link>

                <div className="rounded-2xl border border-slate-200 bg-white p-6">
                    <h2 className="text-lg font-black text-[#12213e]">Tambah FAQ</h2>
                    ...
                    >
                        <div>
                            <label htmlFor="question" className="text-sm font-medium text-[#12213e]">Pertanyaan</label>
                            <input
                                id="question"
                                type="text"
                                value={data.question}
                            ...
                            />
                            {errors.question && <p className="mt-1 text-xs text-rose-600">{errors.question}</p>}
                        </div>

                        <div>
                            <label htmlFor="answer" className="text-sm font-medium text-[#12213e]">Jawaban</label>
                            <textarea
                                id="answer"
                                rows={5}
                                value={data.answer}
                            ...
                            />
                            {errors.answer && <p className="mt-1 text-xs text-rose-600">{errors.answer}</p>}
                        </div>

                        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                            <div>
                                <label htmlFor="sort_order" className="text-sm font-medium text-[#12213e]">Urutan tampil</label>
                                <input
                                    id="sort_order"
                                    type="number"
                                    min={0}
                                    ...
                                    className={`mt-1.5 ${inputClass}`}
                                />
                                {errors.sort_order && <p className="mt-1 text-xs text-rose-600">{errors.sort_order}</p>}
                            </div>
                            <label className="flex cursor-pointer items-center gap-2.5 sm:mt-7">
                                ...
                                />
                                <span className="text-sm text-slate-600">Tampilkan di halaman utama</span>
                            </label>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Link href="/verifikator/faq" className="rounded-xl px-4 py-2.5 text-sm font-medium text-slate-600 transition hover:bg-slate-100">
                                Batal
                            </Link>
                        </div>
                    </form>
