...
export default function OpdKeputusan({
    ...
    return (
        <MagangLayout user={user} title="Perlu Keputusan" active="keputusan" navItems={opdNav}>
            <Head title="Perlu Keputusan — OPD" />

            <div className="space-y-6">
                <div className="flex flex-wrap items-end justify-between gap-3">
                    <div>
                        <h2 className="text-xl font-black text-[#12213e]">Perlu Keputusan</h2>
                        ...
                        {filtered.length > 0 ? (
                            <div className="space-y-2.5">
                                {filtered.map((app) => {
                                    ...
                                    const days = app.forwarded_at ? waitingDays(app.forwarded_at) : 0;
                            ...
                            <div className="flex flex-col items-center gap-2 rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-14 text-center">
                                ...
                                <p className="text-sm font-semibold text-[#12213e]">
                                    {queue.length === 0 ? 'Tidak ada antrian 🎉' : 'Tidak ada hasil pencarian'}
                                </p>
                                <p className="text-sm text-slate-500">
                                    {queue.length === 0 ? 'Semua pengajuan sudah diputuskan.' : 'Coba kata kunci lain.'}
                                </p>
                            </div>
                        )}
                    </div>
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
                            {selected ? (
                                <DecisionPanel
                                    key={selected.id}
                                    app={selected}
                                    onApproved={(id) => removeFromQueue(id, 'Pengajuan disetujui. Peserta dapat mulai magang.')}
                                ...
                                <div className="flex flex-col items-center gap-3 py-16 text-center">
                                    ...
                                    <p className="text-sm font-semibold text-[#12213e]">Pilih pengajuan untuk diputuskan</p>
                                    <p className="max-w-xs text-sm text-slate-500">
                                        Detail penempatan dan tombol keputusan akan muncul di sini.
                                    </p>
                                </div>
                            )}
                        </div>
