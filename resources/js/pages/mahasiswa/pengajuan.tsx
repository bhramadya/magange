...
function buildEvents(app: InternshipApplication): ActivityEvent[] {
    ...
    if (rejectedAtVerif) {
        events.push({
            ...
            detail: app.rejection_reason ?? 'Pengajuan belum dapat disetujui oleh Admin Verifikator.',
    ...
    if (rejectedAtOpd) {
        events.push({
            title: 'Ditolak OPD',
            detail: app.rejection_reason ?? 'OPD tujuan belum dapat menerima penempatan ini.',
            at: app.opd_decision_at,
            icon: XCircle,
            state: 'rejected',
        });
    ...
    const opdDecided = s === 'approved' || s === 'ongoing' || s === 'completion_submitted' || s === 'completed';
    events.push({
        title: 'Disetujui OPD',
        detail: app.division ? `Penempatan: ${app.division}.` : 'OPD tujuan menyetujui penempatan.',
        at: app.opd_decision_at,
        icon: Building2,
        state: opdDecided ? 'done' : s === 'forwarded_opd' ? 'current' : 'upcoming',
    });

    // Pelaksanaan
    const ongoingOrLater = s === 'ongoing' || s === 'completion_submitted' || s === 'completed';
    events.push({
        title: 'Pelaksanaan Magang',
        detail: `Periode ${formatDate(app.start_date)} – ${formatDate(app.end_date)}.`,
        at: app.start_date,
        icon: GraduationCap,
        state: s === 'completed' ? 'done' : ongoingOrLater ? 'current' : 'upcoming',
    });
    ...
    events.push({
        ...
        icon: FileText,
        state: reportDone ? 'done' : s === 'completion_submitted' ? 'current' : 'upcoming',
    });
    ...
    return (
        <ol className="relative">
            {events.map((ev, i) => {
                ...
                const Icon =
                    ev.state === 'current' ? Loader2 : ev.state === 'done' ? CheckCircle2 : ev.state === 'rejected' ? XCircle : ev.icon;
                ...
                const line = ev.state === 'done' ? 'bg-emerald-400' : 'bg-slate-200';
                const titleColor =
                    ev.state === 'done'
                        ? 'text-emerald-700'
                        : ev.state === 'current'
                ...
                return (
                    <li key={ev.title} className="flex gap-4 pb-6 last:pb-0">
                        <div className="relative flex flex-col items-center">
                            <span className={`flex size-9 shrink-0 items-center justify-center rounded-full ring-4 ring-white ${node}`}>
                                <Icon className={`size-[18px] ${ev.state === 'current' ? 'animate-spin' : ''}`} />
                            </span>
                            {!isLast && <span className={`mt-1 w-0.5 flex-1 ${line}`} />}
                        </div>

                        <div className="pt-1">
                            <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
                                <p className={`text-sm font-semibold ${titleColor}`}>{ev.title}</p>
                                {ev.at && <span className="text-[11px] font-medium text-slate-400">· {formatDateTime(ev.at)}</span>}
                            </div>
    ...
    if (s === 'ongoing' || s === 'completion_submitted') {
        ...
        return (
            <div className="rounded-2xl border border-[#cddcef] bg-gradient-to-br from-[#106feb] to-[#0b4fb0] p-5 text-white">
                ...
                >
                    <Upload className="size-4" /> {submitted ? 'Lihat Status Laporan' : 'Ke Penyelesaian'}
                </Link>
            </div>
        );
    }
    ...
    if (s === 'completed') {
        return (
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5">
                ...
                <p className="mt-2 text-sm text-emerald-700/90">e-Sertifikat Anda siap diunduh di halaman Penyelesaian.</p>
                <Link
                    href="/penyelesaian"
    ...
    return (
        <div className="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <p className="mt-2 text-sm text-slate-500">
                Pengajuan Anda sedang ditinjau. Notifikasi perubahan status dikirim via email/WhatsApp.
            </p>
            <Link
    ...
    return (
        <div className="flex items-center gap-3 rounded-xl border border-slate-200 bg-white p-3">
            ...
            <div className="min-w-0 flex-1">
                <p className="text-sm font-semibold text-[#12213e]">{doc.label}</p>
            ...
            {doc.url ? (
                ...
                <span className="shrink-0 text-[11px] font-medium text-slate-400">Terunggah</span>
            )}
        </div>
    );
}
...
function EmptyState() {
    return (
        <div className="mt-8 flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center">
            ...
            <h2 className="mt-4 text-lg font-bold text-[#12213e]">Belum ada pengajuan</h2>
            <p className="mt-1 max-w-sm text-sm text-slate-500">
                Anda belum mengajukan permohonan magang. Mulai sekarang dan pantau prosesnya di sini.
            </p>
            <Link
                href="/pengajuan/baru"
    ...
    return (
        <MagangLayout user={user} title="Pengajuan Saya" active="pengajuan">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <p className="text-sm text-slate-500">Detail permohonan magang Anda</p>
                    <h2 className="text-2xl font-black text-[#12213e]">Pengajuan Saya</h2>
                </div>
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-3">
                ...
                >
                    <div className="rounded-2xl border border-slate-200 bg-white p-6">
                        <h3 className="text-base font-bold text-[#12213e]">Riwayat Aktivitas</h3>
                        <p className="mt-0.5 text-sm text-slate-500">Perjalanan pengajuan Anda lengkap dengan waktu kejadian.</p>
                        <div className="mt-6">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-6">
                        <h3 className="text-base font-bold text-[#12213e]">Dokumen Pengajuan</h3>
                        <p className="mt-0.5 text-sm text-slate-500">Berkas yang Anda lampirkan saat mengajukan.</p>
                        {documents.length > 0 ? (
                            ...
                            <p className="mt-4 rounded-xl bg-slate-50 px-4 py-3 text-sm text-slate-500">Tidak ada dokumen terlampir.</p>
                        )}
                    </div>
                </motion.div>
                ...
                <div className="flex flex-col gap-6">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-6">
                        <h3 className="text-base font-bold text-[#12213e]">Detail Pemohon</h3>
                        <dl className="mt-4 grid grid-cols-1 gap-4">
                            ...
                            <DetailRow label="Durasi" value={`${application.duration_months} Bulan`} />
                            <DetailRow
                                label="Periode"
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-6">
                        <h3 className="text-base font-bold text-[#12213e]">Penempatan</h3>
                        <p className="mt-0.5 text-xs text-slate-500">Ditetapkan Admin Verifikator saat diteruskan ke OPD.</p>
                        <dl className="mt-4 grid grid-cols-1 gap-4">
                            <DetailRow label="OPD Penempatan" value={application.opd?.name ?? null} />
                            <DetailRow label="Bidang/Divisi" value={application.division} />
                            <DetailRow label="Pembimbing Lapangan" value={application.field_supervisor} />
