...
## Current phase: domain action routes wired, dashboard index routes still previews

`routes/web.php` is now **split in two halves** — read its comment blocks before touching routing:

1. **Dashboard/feature index routes (still previews).** The `GET` pages — `dashboard`, `verifikator`, `opd`, and their sub-pages, plus `lacak`/`bantuan`/`pengaturan` — are still **temporary `Route::inertia()` previews**: no auth, no middleware, no controller props. Each renders from its own hard-coded mock data so the whole navigation is clickable during UI review. These still need to be swapped for controllers returning `Inertia::render(...)` with real props (and wrapped in `auth` + `role:` middleware), **keeping the same paths** so frontend `href`s don't change. Don't strip a page's mock data until the matching backend prop is in place.

2. **Domain action routes (real backend, done).** The state-transition endpoints are fully wired: `POST verifikator/pengajuan/{application}/{forward,reject,complete}`, `POST opd/pengajuan/{application}/{approve,reject,complete}`, `POST mahasiswa/pengajuan/{application}/laporan`, `PATCH kuota/{opd}`, plus the OTP (`otp/send`, `otp/verify`) and admin (`admin/login`) auth endpoints. These use real controllers under `app/Http/Controllers/{Auth,Mahasiswa,Opd,Verifikator}/` that delegate to the services, guarded by `auth` + the `role:` middleware and validated through FormRequests in `app/Http/Requests/`.

The **domain layer** underneath is built and tested — models, enums, migrations, seeders (`database/seeders/`: `AdminSeeder`, `OpdSeeder`, `FaqSeeder`), and a full service/job/mail layer (see *Domain service layer* below).
