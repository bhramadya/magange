...
2. **Still** **Route::inertia()** **previews.** Only two prop-less pages remain hard-coded previews: bantuan and pengaturan. login-otp is now a real controller GET (OtpLoginController@showForm). The landing page welcome/home is real (HomeController@index, injecting active faqs + opds), and lacak is a real GET on Mahasiswa\ApplicationController@track (public status lookup via ?email=). Don't strip a preview page's mock data until its backend prop is in place.
